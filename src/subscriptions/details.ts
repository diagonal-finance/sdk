...
export const subscriptionDetails = async (user: string, serviceAddress: string, superTokenAddress: string): Promise<SubscriptionDetails> => {
    ...
    const subscriberState = await serviceContract.getSubscriberState(user, superTokenAddress);
    ...
    const subscriptionDetails: SubscriptionDetails = {
        ...
        subscriberPackageIds: subscriberState.subscriberPackageIds.map(item => item.toNumber()),
...
export const isSubscribedToPackage = async (user: string, serviceAddress: string, packageId: number, superTokenAddress: string): Promise<boolean> => {

    const subDetails = await subscriptionDetails(user, serviceAddress, superTokenAddress);
